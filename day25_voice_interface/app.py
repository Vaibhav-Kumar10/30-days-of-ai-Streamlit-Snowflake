...
with st.sidebar:
    ...
    with st.expander("Database Configuration", expanded=False):
        database = st.text_input(
            "Database", value=st.session_state.voice_database, key="db_input"
        )
        schema = st.text_input(
            "Schema", value=st.session_state.voice_schema, key="schema_input"
        )
        ...
        st.caption(
            ":material/edit_note: Stage uses server-side encryption (required for AI_TRANSCRIBE)"
        )

        # Manual stage recreation button
        if st.button(
            ":material/autorenew: Recreate Stage",
            help="Drop and recreate the stage with correct encryption",
        ):
            try:
    ...
    with st.expander("Stage Status", expanded=False):
        ...
        try:
            ...
            stage_info = session.sql(
                f"SHOW STAGES LIKE 'VOICE_AUDIO' IN SCHEMA {database}.{schema}"
            ).collect()
            ...
            st.success(
                f":material/check_box: Audio stage ready (server-side encrypted)"
            )
            ...
            if transcript:
                ...
                with st.spinner(":material/smart_toy: Generating response..."):
                    ...
                    history_messages = (
                        ...
                        if len(st.session_state.voice_messages) > 1
                        else []
                    )
                    ...
                    history_messages = [
                        ...
                        if not (
                            msg["role"] == "assistant"
                            and "Click the microphone button" in msg["content"]
                        )
                    ]
