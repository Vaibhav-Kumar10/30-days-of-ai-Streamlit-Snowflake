...
AGENT_ENDPOINT = (
    f"/api/v2/databases/{DB_NAME}/schemas/{SCHEMA_NAME}/agents/{AGENT_NAME}:run"
)
                ...
                if line and line.decode("utf-8").startswith("data: "):
                    ...
                    try:
                        ...
                        if event_type == "response" and not result["thinking"]:
                            ...
                            for content_item in content_list:
                                if (
                                    "thinking" in content_item
                                    and not result["thinking"]
                                ):
                                    thinking_obj = content_item.get("thinking", {})
                                    if isinstance(thinking_obj, dict):
                                        result["thinking"] = thinking_obj.get(
                                            "text", ""
                                        )
                                    elif isinstance(thinking_obj, str):
                                        result["thinking"] = thinking_obj
...
with st.sidebar:
    ...
    st.text_input(
        "Database.Schema:", f"{DB_NAME}.{SCHEMA_NAME}", disabled=True, key="search_db"
    )
    ...
    st.text_input(
        "Model File:", "sales_metrics_model.yaml", disabled=True, key="analyst_model"
    )
    st.text_input("Stage:", "MODELS", disabled=True, key="analyst_stage")
    st.text_input(
        "Database.Schema:", f"{DB_NAME}.{SCHEMA_NAME}", disabled=True, key="analyst_db"
    )
...
st.write(
    "The agent uses **orchestration** to automatically choose between Cortex Search (conversations) and Cortex Analyst (metrics)."
)
...
try:
    ...
    if AGENT_NAME in agent_names:
        st.success(
            f"✅ Connected to agent: **{AGENT_NAME}**", icon=":material/check_circle:"
        )
    else:
...
try:
    ...
    with col1:
        st.info(
            f"💬 Conversations: **{convo_count}** records"
            if convo_count > 0
            else "⚠️ No conversation data"
        )
    with col2:
        if metrics_count > 0:
    ...
    with st.chat_message(msg["role"]):
        if msg["role"] == "assistant":
            ...
            if msg.get("tool_name") and msg.get("tool_type"):
                st.caption(
                    f":material/build: Tool: **{msg['tool_name']}** (`{msg['tool_type']}`)"
                )
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        if result["tool_name"] and result["tool_type"]:
            st.caption(
                f":material/build: Tool: **{result['tool_name']}** (`{result['tool_type']}`)"
            )
...
st.divider()
st.caption(
    "Day 27: Multi-Tool Agent Orchestration | Chat with Sales Data | 30 Days of AI with Streamlit"
)
