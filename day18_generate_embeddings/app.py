...
with st.container(border=True):
    ...
    with col1:
        st.session_state.day18_database = st.text_input(
            "Database", value=st.session_state.day18_database, key="day18_db_input"
        )
    with col2:
        st.session_state.day18_schema = st.text_input(
            "Schema", value=st.session_state.day18_schema, key="day18_schema_input"
        )
    with col3:
    ...
    st.info(
        f":material/location_on: Loading from: `{st.session_state.day18_database}.{st.session_state.day18_schema}.{st.session_state.day18_chunk_table}`"
    )
    st.caption(
        ":material/lightbulb: This should point to the REVIEW_CHUNKS table from Day 17"
    )

    # Check for existing loaded data
    if "chunks_data" in st.session_state:
        st.success(
            f":material/check_circle: **{len(st.session_state.chunks_data)} chunk(s)** already loaded"
        )

    # Load chunks button
    if st.button(
        ":material/folder_open: Load Chunks", type="primary", use_container_width=True
    ):
        try:
            with st.status("Loading chunks...", expanded=True) as status:
                ...
                status.update(
                    label="Chunks loaded successfully!",
                    state="complete",
                    expanded=False,
                )
            ...
            st.info(
                ":material/lightbulb: Make sure you've processed reviews in Day 17 first!"
            )
...
if "chunks_data" in st.session_state:
    ...
    with st.container(border=True):
        ...
        **For Customer Reviews**: This allows your RAG system to:
        ...
        if st.button(
            ":material/calculate: Generate Embeddings",
            type="primary",
            use_container_width=True,
        ):
            try:
                with st.status(
                    "Generating embeddings using Snowflake Cortex...", expanded=True
                ):
                    query = f"""
                    SELECT
                        CHUNK_ID,
                    ...
                    ORDER BY CHUNK_ID
                    """
                    df_embeddings = session.sql(query).to_pandas()

                    # Convert to session_state format used later
                    embeddings = [
                    ...
                    st.success(
                        f":material/check_circle: Generated {len(embeddings)} embeddings using batch processing!"
                    )
    ...
    #                         st.write(
    #                             f"Processing chunks {i+1} to {batch_end} of {total_chunks}..."
    #                         )
    #                         for idx, row in df.iloc[i:batch_end].iterrows():
    #                             # Generate embedding using the correct function signature
    #                             emb = embed_text_768(
    #                                 model="snowflake-arctic-embed-m", text=row["CHUNK_TEXT"]
    #                             )
    #                             embeddings.append(
    #                                 {"chunk_id": row["CHUNK_ID"], "embedding": emb}
    #                             )
    #                         # Update progress
    #                         progress = batch_end / total_chunks
    #                         progress_bar.progress(progress)
    #                     status.update(
    #                         label="Embeddings generated!", state="complete", expanded=False
    #                     )
    #                     # Store in session state
    #                     st.session_state.embeddings_data = embeddings
    #                     st.success(
    #                         f":material/check_circle: Generated {len(embeddings)} embeddings for {len(df)} review chunks!"
    #                     )
    #             except Exception as e:
    #                 st.error(f"Error generating embeddings: {str(e)}")
    ...
    if "embeddings_data" in st.session_state:
        ...
        with st.container(border=True):
            ...
            try:
                ...
                if current_count > 0:
                    st.warning(
                        f":material/warning: **{current_count:,} embedding(s)** currently in table `{full_embedding_table}`"
                    )
                    embedding_table_exists = True
                else:
                    st.info(
                        ":material/inbox: **Embedding table is empty** - No embeddings saved yet."
                    )
                    embedding_table_exists = False
            except:
                st.info(
                    ":material/inbox: **Embedding table doesn't exist yet** - Will be created when you save embeddings."
                )
                embedding_table_exists = False
            ...
            if "day18_replace_mode" not in st.session_state:
                ...
                if (
                    "day18_last_embedding_table" not in st.session_state
                    or st.session_state.day18_last_embedding_table
            ...
            if replace_mode:
                st.warning(
                    "**Replace Mode Active**: Existing embeddings will be deleted before saving new ones."
                )
            else:
                st.success(
                    "**Append Mode Active**: New embeddings will be added to existing data."
                )

            if st.button(
                ":material/save: Save Embeddings to Snowflake",
                type="primary",
                use_container_width=True,
            ):
                try:
                    with st.status("Saving embeddings...", expanded=True) as status:
                        ...
                        st.write(
                            f":material/looks_two: Inserting {len(embeddings)} embeddings..."
                        )
                        ...
                        for i, emb_data in enumerate(embeddings):
                            ...
                            emb_array = (
                                "[" + ",".join([str(float(x)) for x in emb_list]) + "]"
                            )
                            ...
                            if (i + 1) % 10 == 0:
                                st.write(
                                    f"Saved {i + 1} of {len(embeddings)} embeddings..."
                                )

                        status.update(
                            label="Embeddings saved!", state="complete", expanded=False
                        )

                    mode_msg = "replaced in" if replace_mode else "saved to"
                    st.success(
                        f":material/check_circle: Successfully {mode_msg} `{full_embedding_table}`\n\n:material/calculate: {len(embeddings)} embedding(s) now in table"
                    )

                    # Store for Day 19
                    st.session_state.embeddings_table = full_embedding_table
                    st.session_state.embeddings_database = (
                        st.session_state.day18_database
                    )
...
with st.container(border=True):
    ...
    try:
        ...
        if count_result:
            ...
            if record_count > 0:
                st.warning(
                    f":material/warning: **{record_count:,} embedding(s)** currently in table `{full_embedding_table}`"
                )
            else:
                st.info(
                    ":material/inbox: **Embedding table is empty** - Generate and save embeddings above."
                )
    except:
        st.info(
            ":material/inbox: **Embedding table doesn't exist yet** - Generate and save embeddings to create it."
        )

    query_button = st.button(
        ":material/analytics: Query Embedding Table",
        type="secondary",
        use_container_width=True,
    )
    ...
    if (
        "queried_embeddings" in st.session_state
        and st.session_state.get("queried_embeddings_table") == full_embedding_table
    ):
        ...
        if len(emb_df) > 0:
            ...
            st.info(
                ":material/lightbulb: Self-distance should be 0, confirming embeddings are stored correctly"
            )
            ...
            if embedding_col:
                with st.expander(":material/search: View Individual Embedding Vectors"):
                    st.write(
                        "Select a CHUNK_ID to view its full 768-dimensional embedding vector:"
                    )
                    ...
                    selected_chunk = st.selectbox(
                        "Select CHUNK_ID", chunk_ids, key="view_embedding_chunk"
                    )

                    if st.button(
                        ":material/analytics: Load Embedding Vector",
                        key="load_embedding_btn",
                    ):
                        # Get the embedding for selected chunk
                        selected_emb = emb_df[emb_df[chunk_id_col] == selected_chunk][
                            embedding_col
                        ].iloc[0]
                    ...
                    if "loaded_embedding" in st.session_state:
                        st.write(
                            f"**Embedding Vector for CHUNK_ID {st.session_state.loaded_embedding_chunk}:**"
                        )
        ...
        st.info(
            ":material/inbox: No embeddings queried yet. Click 'Query Embedding Table' to view saved embeddings."
        )
