...
st.write(
    "Create a semantic search service for the customer reviews processed in Days 16-18."
)
...
with st.container(border=True):
    ...
    with col1:
        st.session_state.day19_database = st.text_input(
            "Database", value=st.session_state.day19_database, key="day19_db_input"
        )
    with col2:
        st.session_state.day19_schema = st.text_input(
            "Schema", value=st.session_state.day19_schema, key="day19_schema_input"
        )

    st.info(
        f":material/location_on: Using: `{st.session_state.day19_database}.{st.session_state.day19_schema}`"
    )
    st.caption(
        ":material/lightbulb: Make sure your REVIEW_CHUNKS table exists in this location"
    )
...
""",
    ...
    if st.button(
        ":material/build: Create Search View", type="primary", use_container_width=True
    ):
        try:
            create_view_sql = f"""
            CREATE OR REPLACE VIEW {st.session_state.day19_database}.{st.session_state.day19_schema}.REVIEW_SEARCH_VIEW AS
            ...
            st.success(
                f":material/check_circle: Created view: `{st.session_state.day19_database}.{st.session_state.day19_schema}.REVIEW_SEARCH_VIEW`"
            )
...
:material/lightbulb: **Key Parameters:**
    ...
    warehouse = st.text_input(
        "Warehouse Name", value="COMPUTE_WH", help="Enter your Snowflake warehouse name"
    )

    # Button to create search service
    if st.button(
        ":material/rocket_launch: Create Search Service",
        type="primary",
        use_container_width=True,
    ):
        try:
            with st.status(
                "Creating Cortex Search Service...", expanded=True
            ) as status:
                ...
                status.update(
                    label=":material/check_circle: Search service created!",
                    state="complete",
                    expanded=False,
                )

            st.success(
                f":material/check_circle: Created: `{st.session_state.day19_database}.{st.session_state.day19_schema}.CUSTOMER_REVIEW_SEARCH`"
            )
...
with st.container(border=True):
    ...
    if st.button(
        ":material/assignment: List My Cortex Search Services", use_container_width=True
    ):
        try:
            # Try to show services in the current database/schema
            result = session.sql(
                f"SHOW CORTEX SEARCH SERVICES IN SCHEMA {st.session_state.day19_database}.{st.session_state.day19_schema}"
            ).collect()
            if result:
                st.success(
                    f":material/check_circle: Found {len(result)} Cortex Search service(s) in `{st.session_state.day19_database}.{st.session_state.day19_schema}`:"
                )
                st.dataframe(result, use_container_width=True)
            else:
                st.info(
                    "No Cortex Search services found in this schema. Create one using the button in Step 2!"
                )
            ...
            st.info(
                ":material/lightbulb: If the service was just created, it may take a moment to appear. Try refreshing in a few seconds."
            )
