...
with st.sidebar:
    ...
    db_name, schema_name, agent_name, search_service = (
        "CHANINN_SALES_INTELLIGENCE",
        "DATA",
...
# Tabs
tab0, tab1 = st.tabs(
    [":material/database: Data Setup", ":material/build: Create Agent"]
)
...
"""
    ...
    if st.button(
        ":material/play_arrow: Run Step 1", key="run_step1", use_container_width=True
    ):
        with st.spinner("Creating database and schema..."):
            try:
                for sql in [
                    f'CREATE OR REPLACE DATABASE "{db_name}"',
                    f'CREATE OR REPLACE SCHEMA "{db_name}"."{schema_name}"',
...
-- Insert 10 comprehensive conversation transcripts (See code for full insertion)"""
    ...
    if st.button(
        ":material/play_arrow: Run Step 2", key="run_step2", use_container_width=True
    ):
        with st.spinner("Creating table and inserting data..."):
            try:
                session.sql(
                    f"""CREATE OR REPLACE TABLE "{db_name}"."{schema_name}".SALES_CONVERSATIONS (
                    conversation_id VARCHAR, transcript_text TEXT, customer_name VARCHAR, deal_stage VARCHAR,
                    sales_rep VARCHAR, conversation_date TIMESTAMP, deal_value FLOAT, product_line VARCHAR)"""
                ).collect()

                session.sql(
                    f"""INSERT INTO "{db_name}"."{schema_name}".SALES_CONVERSATIONS 
                ...
                st.success(
                    "✓ Step 2 complete! Table created with 10 comprehensive conversation transcripts"
                )
            except Exception as e:
                st.error(f"Error: {e}")
...
CREATE CORTEX SEARCH SERVICE IF NOT EXISTS "{db_name}"."{schema_name}".{search_service}
  ...
  AS (SELECT transcript_text, customer_name, deal_stage, sales_rep, conversation_date
    ...
    if st.button(
        ":material/play_arrow: Run Step 3", key="run_step3", use_container_width=True
    ):
            ...
            try:
                ...
                try:
                    existing = session.sql(
                        f'SHOW CORTEX SEARCH SERVICES IN SCHEMA "{db_name}"."{schema_name}"'
                    ).collect()
                    service_exists = any(
                        row["name"] == search_service for row in existing
                    )
                except:
                ...
                if service_exists:
                    st.write(
                        f":material/check_circle: Search service '{search_service}' already exists"
                    )
                    status.update(
                        label="✓ Step 3 complete (service already exists)!",
                        state="complete",
                    )
                else:
                    ...
                    session.sql(
                        f'ALTER TABLE "{db_name}"."{schema_name}".SALES_CONVERSATIONS SET CHANGE_TRACKING = TRUE'
                    ).collect()

                    # Step 3.3: Create search service
                    st.write(
                        ":material/build: Creating Cortex Search service (this takes 30-60 seconds)..."
                    )
                    session.sql(
                        f"""CREATE CORTEX SEARCH SERVICE "{db_name}"."{schema_name}".{search_service}
                        ON transcript_text ATTRIBUTES customer_name, deal_stage, sales_rep WAREHOUSE = COMPUTE_WH TARGET_LAG = '1 hour'
                        AS (SELECT transcript_text, customer_name, deal_stage, sales_rep, conversation_date
                            FROM "{db_name}"."{schema_name}".SALES_CONVERSATIONS WHERE conversation_date >= '2024-01-01')"""
                    ).collect()
                    ...
                    st.write(
                        ":material/check_circle: Search service created successfully"
                    )
                    status.update(
                        label="✓ Step 3 complete! Service is indexing in background (1-2 min)",
                        state="complete",
                    )
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    st.info(
        "**Sales Metrics Table** contains structured deal data that Cortex Analyst will query.\nThis data will be used in Day 28 for natural language SQL generation."
    )
...
-- Insert sample sales metrics data (10 deals)"""
    ...
    if st.button(
        ":material/play_arrow: Run Step 4", key="run_step4", use_container_width=True
    ):
        with st.spinner("Creating sales metrics table..."):
            try:
                session.sql(
                    f"""CREATE OR REPLACE TABLE "{db_name}"."{schema_name}".SALES_METRICS (
                    deal_id VARCHAR, customer_name VARCHAR, deal_value FLOAT, close_date DATE,
                    sales_stage VARCHAR, win_status BOOLEAN, sales_rep VARCHAR, product_line VARCHAR)"""
                ).collect()
                session.sql(
                    f"""INSERT INTO "{db_name}"."{schema_name}".SALES_METRICS VALUES
                ...
                st.success(
                    "✓ Step 4 complete! Sales metrics table created with 10 deals"
                )
            except Exception as e:
                st.error(f"Error: {e}")
...
"""
    ...
    with col1:
        st.download_button(
            ":material/download: Download YAML",
            semantic_model_yaml,
    ...
    with col2:
        if st.button(
            ":material/cloud_upload: Auto-Upload to Stage",
            key="run_step5",
            use_container_width=True,
            type="primary",
        ):
            with st.spinner("Creating stage and uploading YAML..."):
                try:
                    ...
                    session.sql(
                        f'CREATE STAGE IF NOT EXISTS "{db_name}"."{schema_name}".MODELS'
                    ).collect()

                    # Clean up old files
                    try:
                        files = session.sql(
                            f'LIST @"{db_name}"."{schema_name}".MODELS'
                        ).collect()
                        for row in files:
                            fname = str(row["name"]).split("/")[-1]
                            if "sales_metrics_model" in fname.lower():
                                session.sql(
                                    f'REMOVE @"{db_name}"."{schema_name}".MODELS/{fname}'
                                ).collect()
                    except:
                        pass
                    ...
                    try:
                        ...
                        uploaded_files = [
                            str(row["name"]).split("/")[-1] for row in files
                        ]
                        if "sales_metrics_model.yaml" in uploaded_files:
                            st.success(
                                "✓ Step 5 complete! YAML uploaded as `sales_metrics_model.yaml`"
                            )
                        else:
                            found_file = next(
                                (
                                    ...
                                    if "sales_metrics_model" in f.lower()
                                ),
                                None,
                            )
                            if found_file:
                                st.warning(
                                    f"⚠️ File uploaded as `{found_file}` instead of `sales_metrics_model.yaml`"
                                )
                                st.info("Day 28 will automatically detect this file.")
                            else:
                                st.error(
                                    "Upload succeeded but file not found in stage listing"
                                )
                    finally:
                        try:
                    ...
                    st.info(
                        "💡 Use the 'Download YAML' button and upload manually via Snowsight instead"
                    )
    ...
    if st.button(
        ":material/verified: Check if Data is Ready",
        type="primary",
        use_container_width=True,
    ):
        with st.status("Verifying setup...", expanded=True) as status:
            all_good = True
            checks = [
                (f"SHOW DATABASES LIKE '{db_name}'", "Database exists"),
                # (f'USE DATABASE "{db_name}"', "Database exists"),
                (
                    f'SELECT COUNT(*) as cnt FROM "{db_name}"."{schema_name}".SALES_CONVERSATIONS',
                    "Conversations table",
                    True,
                ),
                (
                ...
                (
                    f'SELECT COUNT(*) as cnt FROM "{db_name}"."{schema_name}".SALES_METRICS',
                    "Sales metrics table",
                    True,
                    None,
                    True,
                ...
                (
                    f'SHOW STAGES IN SCHEMA "{db_name}"."{schema_name}"',
                    "MODELS stage",
                    False,
                    "MODELS",
                    True,
            ...
            for check in checks:
                ...
                try:
                    ...
                    if len(check) > 2 and check[2]:  # Count query
                        st.write(
                            f":material/check_circle: {name} with {result[0]['CNT']} records"
                        )
                        ...
                        if found:
                            ...
                            st.write(
                                f":material/{'warning' if len(check) > 4 else 'cancel'}: {name} not found"
                            )
                            if len(check) <= 4:
                                all_good = False
                    else:
            ...
            if all_good:
                ...
                status.update(
                    label="Complete Steps 1-3 for Day 27, Steps 4-5 for Day 28+",
                    state="error",
                )
...
- For conversation questions (summaries, discussions), use the ConversationSearch tool"""
  ...
  $$;"""
    ...
    if st.button(
        ":material/play_arrow: Create Agent", type="primary", use_container_width=True
    ):
        try:
            with st.status("Creating agent...") as status:
                try:
                    ...
                    if "syntax error" in str(e).lower():
                        st.error(
                            ":material/error: Cortex Agents not available in your account"
                        )
                ...
                status.update(
                    label=":material/check_circle: Agent Ready!", state="complete"
                )
                st.balloons()
        except Exception as e:
...
st.divider()
st.caption(
    "Day 26: Introduction to Cortex Agents | Create Your First Agent | 30 Days of AI with Streamlit"
)
