...
st.write(
    "Load customer reviews from Day 16, process them, and prepare searchable chunks for RAG."
)
...
with st.container(border=True):
    ...
    with col1:
        st.session_state.day17_database = st.text_input(
            "Database", value=st.session_state.day17_database, key="day17_db_input"
        )
    with col2:
        st.session_state.day17_schema = st.text_input(
            "Schema", value=st.session_state.day17_schema, key="day17_schema_input"
        )
    with col3:
    ...
    st.info(
        f":material/location_on: Loading from: `{st.session_state.day17_database}.{st.session_state.day17_schema}.{st.session_state.day17_table_name}`"
    )
    st.caption(
        ":material/lightbulb: This should point to the EXTRACTED_DOCUMENTS table from Day 16"
    )

    # Check for existing loaded data
    if "loaded_data" in st.session_state:
        st.success(
            f":material/check_circle: **{len(st.session_state.loaded_data)} document(s)** already loaded"
        )

    # Load documents button
    if st.button(
        ":material/folder_open: Load Reviews", type="primary", use_container_width=True
    ):
        try:
            with st.status(
                "Loading reviews from Snowflake...", expanded=True
            ) as status:
                ...
                status.update(
                    label="Reviews loaded successfully!",
                    state="complete",
                    expanded=False,
                )
            ...
            st.info(
                ":material/lightbulb: Make sure you've uploaded review files in Day 16 first!"
            )
...
if "loaded_data" in st.session_state:
    with st.container(border=True):
        ...
        st.dataframe(
            df[["DOC_ID", "FILE_NAME", "FILE_TYPE", "UPLOAD_TIMESTAMP", "WORD_COUNT"]],
    ...
    with st.container(border=True):
        ...
        **Customer Review Processing Options:**
        ...
        if "Chunk reviews" in processing_option:
            ...
            st.caption(
                f"Reviews with >{chunk_size} words will be split into chunks of {chunk_size} words with {overlap} word overlap"
            )
        else:
        ...
        if st.button(
            ":material/flash_on: Process Reviews",
            type="primary",
            use_container_width=True,
        ):
            chunks = []
            ...
            with st.status("Processing reviews...", expanded=True) as status:
                if "Keep each review" in processing_option:
                    ...
                    st.write(
                        f":material/check_circle: Created {len(chunks)} chunks (1 per review)"
                    )

                else:
                    # Option 2: Chunk longer reviews
                    st.write(
                        f":material/edit_note: Chunking reviews longer than {chunk_size} words..."
                    )
                    chunk_id = 1
                    ...
                    st.write(
                        f":material/check_circle: Created {len(chunks)} chunks from {len(df)} reviews"
                    )
            ...
            st.success(
                f":material/check_circle: Processed {len(df)} reviews into {len(chunks)} searchable chunks!"
            )
    ...
    if "review_chunks" in st.session_state:
        with st.container(border=True):
            ...
            with col2:
                full_reviews = len(
                    [c for c in chunks if c["chunk_type"] == "full_review"]
                )
                st.metric("Full Reviews", full_reviews)
            with col3:
                split_reviews = len(
                    [c for c in chunks if c["chunk_type"] == "chunked_review"]
                )
        ...
        with st.container(border=True):
            ...
            try:
                ...
                if count_result:
                    ...
                    if record_count > 0:
                        st.warning(
                            f":material/warning: **{record_count} chunk(s)** currently in table `{full_chunk_table}`"
                        )
                        chunk_table_exists = True  # Only tick if table has data
                    else:
                        st.info(
                            ":material/inbox: **Chunk table is empty** - No chunks saved yet."
                        )
                        chunk_table_exists = False
            except:
                st.info(
                    ":material/inbox: **Chunk table doesn't exist yet** - Will be created when you save chunks."
                )
                chunk_table_exists = False
            ...
            if "day17_replace_mode" not in st.session_state:
                ...
                if (
                    "day17_last_chunk_table" not in st.session_state
                    or st.session_state.day17_last_chunk_table != full_chunk_table
                ):
                    st.session_state.day17_replace_mode = chunk_table_exists
            ...
            if replace_mode:
                st.warning(
                    "**Replace Mode Active**: Existing chunks will be deleted before saving new ones."
                )
            else:
                st.success(
                    "**Append Mode Active**: New chunks will be added to existing data."
                )

            # Save chunks to table
            if st.button(
                ":material/save: Save Chunks to Snowflake",
                type="primary",
                use_container_width=True,
            ):
                try:
                    with st.status(
                        "Saving chunks to Snowflake...", expanded=True
                    ) as status:
                        ...
                        if replace_mode:
                            st.write(
                                ":material/sync: Replace mode: Clearing existing chunks..."
                            )
                            try:
                                session.sql(
                                    f"TRUNCATE TABLE {full_chunk_table}"
                                ).collect()
                                st.write(
                                    "   :material/check_circle: Existing chunks cleared"
                                )
                            except Exception as e:
                                st.write(
                                    f"   :material/warning: No existing chunks to clear"
                                )

                        # Step 3: Insert chunks
                        st.write(
                            f":material/looks_3: Inserting {len(chunks)} chunk(s)..."
                        )
                        chunks_df = pd.DataFrame(chunks)

                        # Rename columns to uppercase to match Snowflake table
                        chunks_df_upper = chunks_df[
                            [
                                "chunk_id",
                                "doc_id",
                                "file_name",
                        ...
                        status.update(
                            label=":material/check_circle: Chunks saved!",
                            state="complete",
                            expanded=False,
                        )
                    ...
                    st.success(
                        f":material/check_circle: Successfully {mode_msg} `{full_chunk_table}`\n\n:material/description: {len(chunks)} chunk(s) now in table"
                    )
...
with st.container(border=True):
    ...
    query_button = st.button(
        ":material/analytics: Query Chunk Table",
        type="secondary",
        use_container_width=True,
    )
    ...
    if (
        "queried_chunks" in st.session_state
        and st.session_state.get("queried_chunks_table") == full_chunk_table
    ):
        ...
        if len(chunks_df) > 0:
            ...
            with col3:
                split_count = len(
                    chunks_df[chunks_df["CHUNK_TYPE"] == "chunked_review"]
                )
            ...
            with st.expander(":material/menu_book: View Full Chunk Text"):
                ...
                if st.session_state.get("load_chunk_text") and st.session_state.get(
                    "selected_chunk_id"
                ):
        ...
        st.info(
            ":material/inbox: No chunks queried yet. Click 'Query Chunk Table' to view saved chunks."
        )
