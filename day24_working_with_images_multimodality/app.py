...
with st.sidebar:
    ...
    with st.expander("Database Configuration", expanded=False):
        database = st.text_input(
            "Database", value=st.session_state.image_database, key="img_db_input"
        )
        schema = st.text_input(
            "Schema", value=st.session_state.image_schema, key="img_schema_input"
        )

        if (
            database != st.session_state.image_database
            or schema != st.session_state.image_schema
        ):
    ...
    with st.expander("Stage Status", expanded=False):
        ...
        try:
            ...
            stage_info = session.sql(
                f"SHOW STAGES LIKE 'IMAGE_ANALYSIS' IN {database}.{schema}"
            ).collect()
...
st.write(
    "Upload an image and let AI analyze it using Snowflake's `AI_COMPLETE` function."
)
        ...
        if st.button(":material/search: Analyze Image", type="primary"):
            ...
            with st.spinner(f":material/psychology: Analyzing with {model}..."):
                try:
                    ...
                    st.info(
                        ":material/lightbulb: Make sure your Snowflake account has access to vision-capable models and that the stage has server-side encryption."
                    )
                    st.stop()
