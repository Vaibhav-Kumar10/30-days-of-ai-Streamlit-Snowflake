...
with st.container(border=True):
    ...
    if available_services:
        ...
        if search_service_option == "-- Enter manually --":
            search_service = st.text_input(
                "Enter service path:", placeholder="database.schema.service_name"
            )
        else:
    ...
    st.caption(
        ":material/lightbulb: This should point to your CUSTOMER_REVIEW_SEARCH service from Day 19"
    )
    ...
    search_clicked = st.button(
        ":material/search: Search", type="primary", use_container_width=True
    )
    ...
    if search_clicked:
        if query and search_service:
            try:
                ...
                if len(parts) != 3:
                    st.error(
                        "Service path must be in format: database.schema.service_name"
                    )
                else:
                    svc = (
                    ...
                    st.success(
                        f":material/check_circle: Found {len(results.results)} result(s)!"
                    )
                    ...
                    for i, item in enumerate(results.results, 1):
                        with st.container(border=True):
                            ...
                            with col1:
                                st.markdown(
                                    f"**Result {i}** - {item.get('FILE_NAME', 'N/A')}"
                                )
                            with col2:
            ...
            ### 8. Error Handling and Empty States
                ...
                st.info(
                    ":material/lightbulb: **Troubleshooting:**\n- Make sure the search service exists (check Day 19)\n- Verify the service has finished indexing\n- Check that you have access permissions"
                )
        else:
            st.warning(
                ":material/warning: Please enter a query and configure a search service."
            )
            st.info(
                ":material/lightbulb: **Need a search service?**\n- Complete Day 19 to create `CUSTOMER_REVIEW_SEARCH`\n- The service will automatically appear in the dropdown above"
            )
    else:
        st.info(
            ":material/arrow_upward: Configure your search service and enter a query above, then click Search to see results."
        )
