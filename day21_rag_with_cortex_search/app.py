...
with st.sidebar:
    ...
    num_chunks = st.slider(
        "Context chunks:", 1, 10, 3, help="Number of relevant chunks to retrieve"
    )
    ...
    if question and search_service:
        ...
        with st.status("Processing...", expanded=True) as status:
            ...
            try:
                ...
                st.write(
                    f"   :material/check_circle: Found {len(context_chunks)} relevant chunks"
                )
...
Provide a clear, accurate answer based on the context. If you use information from the context, mention it naturally."""
                ...
                if show_context:
                    ...
                    st.caption(
                        f"Used {len(context_chunks)} chunks from customer reviews"
                    )
                    for i, (chunk, source) in enumerate(
                        zip(context_chunks, sources), 1
                    ):
                        with st.expander(
                            f":material/description: Chunk {i} - {source}"
                        ):
                            st.write(chunk)
        ...
        st.warning(
            ":material/warning: Please enter a question and configure a search service."
        )
        st.info(
